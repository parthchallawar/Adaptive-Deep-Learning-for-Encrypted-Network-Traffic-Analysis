...
- **Status:** partially implemented (D3/D4 downloaders, manifest and D1 Path B exporter built and tested; D4 downloaded and exported for real, 403,394 flows; D1's exporter run for real against one day only — the weeks 11-52 corpus is a Kaggle-kernel job, spec's own Path B decision, out of phase 1's scope; D3 registration complete as of 2026-09-21, two of five real archives downloaded and exported, 130,303 real flows across 8 classes, real throughput reverses the earlier per-class-subset decision — see the Open questions correction below; remaining D3 archives not yet fetched; D2 not yet acquired; D5 deferred to phase 5 by design)
...
**Correction (2026-09-21), D3 real archive shape:** the registration was completed and the real post-registration `/PCAPs` listing turned out to serve **zip archives** (`VPN-PCAPs-01/02.zip`, `NonVPN-PCAPs-01/02/03.zip`), not individual pcaps directly as `discover_files` originally assumed (matching USTC's flat-listing shape, which was the only real precedent available at the time). `iscx_download.py` now discovers `*.zip` links and extracts them with a new `adl_etc.data.download.extract_zip`, mirroring `extract_7z`'s design. A second real finding followed immediately: 11 of 23 files in the real `NonVPN-PCAPs-01.zip` are `.pcapng` (the newer capture format), not `.pcap` — `extract_zip`'s default now matches both suffixes, and a matching bug in `export_pcap.py`'s file-discovery glob (`*.pcap` only, silently exporting zero of any `.pcapng` file with no error) was found and fixed the same way.

ISCX ships no per-flow label column, so `scripts/download_iscx.py` infers each file's class (7 categories x VPN/non-VPN, spec 001's D3 row) from its file name via `adl_etc.data.iscx_labels`, using the dataset's documented category grouping plus the VPN-substring convention. This was a **documented heuristic, written and tested against plausible file names reconstructed from the widely-cited ISCX naming convention, not verified against the real file listing** — until 2026-09-21, when it was: all 37 real file names across the two archives downloaded so far (`VPN-PCAPS-01.zip`, `NonVPN-PCAPs-01.zip`) resolved correctly, **zero unresolved**. Every row in `labels.csv` still carries a `label_confidence` column (`"heuristic"`) — the heuristic has now been checked against real names from two of five archives, not all of them, so the label is left as-is rather than upgraded to "confirmed" on partial evidence. A file the heuristic cannot classify gets an empty `class_name` rather than a guess, and the downloader prints every such file so a human reviews it.
...
- ~~ISCX subset size for the category-level transfer experiment (default: all pcaps, since disk allows it).~~ **Resolved (2026-09-20), then corrected (2026-09-21) once real ISCX data existed to measure instead of extrapolate from a proxy.** The 2026-09-20 estimate (~1.7 MB/s, ~281 minutes for 28 GB) was measured on real D4 captures because no ISCX file existed yet — the best available proxy at the time, but a proxy, and it was wrong for this corpus: D4's `Neris.pcap` is a botnet capture with many short, bursty flows, which is not representative of ISCX's chat/voip/p2p/video traffic. Once the user registered and this project processed two real ISCX archives (`VPN-PCAPS-01.zip` + `NonVPN-PCAPs-01.zip`, 37 files, 1,786.8 MB), the *real* measured rate was **27.7 MB/s — 16x faster**, extrapolating to **~17 minutes** for the full 28 GB corpus, comfortably under plan T4's 60-minute gate. The "per-class subset, not full corpus" decision this note previously recorded is therefore **reversed**: the full corpus is feasible on the original throughput budget, pending the user fetching the remaining archives. This is exactly the kind of correction the project's own methodology (measure the real thing, not a proxy, the moment the real thing is available) is supposed to produce.
